...
template <typename Shape, typename Derived>
ShapeSample SamplableShape<Shape, Derived>::sample(Sampler& sampler, const proto::Vec3f& from) const {
    auto uv = proto::Vec2f(sampler(), sampler());
...
ShapeSample UniformTriangle::sample_at(proto::Vec2f uv) const {
    ...
    return ShapeSample { uv, pos, normal, inv_area, inv_area };
}

ShapeSample UniformTriangle::sample_at(const proto::Vec2f& uv, const proto::Vec3f&) const {
    return sample_at(uv);
}
...
ShapeSample UniformSphere::sample_at(const proto::Vec2f& uv) const {
    ...
    return ShapeSample { uv, pos, dir, inv_area, inv_area };
}

ShapeSample UniformSphere::sample_at(const proto::Vec2f& uv, const proto::Vec3f&) const {
    return sample_at(uv);
}
